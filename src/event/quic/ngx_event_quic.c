...
#if (NGX_QUICHE)
static ngx_int_t ngx_quiche_new_connection(ngx_connection_t *c,
    ngx_quic_conf_t *conf, ngx_quic_header_t *pkt);
static ngx_int_t ngx_quiche_handle_datagram(ngx_connection_t *c,
    ngx_buf_t *b, ngx_quic_conf_t *conf);
static void ngx_quiche_close_handler(ngx_event_t *ev);
static void ngx_quiche_set_timer(ngx_connection_t *c);
static void ngx_quic_run_quiche(ngx_connection_t *c, ngx_quic_conf_t *conf);
...
{
    if (c->listening->quic == NGX_QUIC_NGINX_VERSION) {
        ngx_quic_do_run(c, conf);

        return;
    }

#if (NGX_QUICHE)
    if (c->listening->quic == NGX_QUIC_QUICHE_VERSION) {
        ngx_quic_run_quiche(c, conf);
    ...
    ngx_log_error(NGX_LOG_ERR, c->log, 0,
                  "quic unknown backend: %ui", c->listening->quic);
}
    ...
    ngx_quic_header_t *pkt)
{
    ...
    qc->explicit_close = ngx_quic_close_connection;
    ...
    qc->streams.stream_init = ngx_quic_init_stream;
    qc->streams.server_streams_left = ngx_quic_server_streams_left;
...
static void
ngx_quic_run_quiche(ngx_connection_t *c, ngx_quic_conf_t *conf)
{
    ngx_int_t               rc;
    ngx_quic_connection_t  *qc;
    ...
    rc = ngx_quiche_handle_datagram(c, c->buffer, NULL);
    if (rc != NGX_OK) {
        ngx_quiche_close_connection(c, NGX_ERROR);
        return;
    }
...
static ngx_int_t
ngx_quiche_new_connection(ngx_connection_t *c, ngx_quic_conf_t *conf,
    ngx_quic_header_t *pkt)
{
    ngx_str_t               odcid;
    ...
    qc->explicit_close = ngx_quiche_close_connection;
    ...
    qc->conf = conf;
    qc->streams.stream_init = ngx_quiche_init_stream;
    qc->streams.server_streams_left = ngx_quiche_server_streams_left;
    ...
    connection = quiche_conn_new_with_tls(qsock->sid.id, qsock->sid.len,
                               odcid.data, odcid.len,
                               c->local_sockaddr, c->local_socklen,
                               c->sockaddr, c->socklen,
                               conf->config, c->ssl->connection, 1);
    if (connection == NULL) {
        return NGX_ERROR;
    }

    quiche_conn_set_max_idle_timeout(connection, conf->idle_timeout);
...
ngx_quiche_input_handler(ngx_event_t *rev)
{
    ...
    if (ngx_quiche_handle_datagram(pc, b, NULL) != NGX_OK) {
        ngx_quiche_close_connection(pc, NGX_ERROR);
        return;
    }
}
    ...
    ngx_quic_conf_t *conf)
{
    ...
    if (!pc->ssl->handshaked) {
            ...
            || quiche_conn_is_in_early_data(qc->connection))
        {
            ...
            if (qc->conf->retry) {
                if (ngx_quiche_connection_send_new_token(pc) != NGX_OK) {
...
ngx_quiche_handle_initial_packet(ngx_connection_t *c, ngx_quic_conf_t *conf)
{
    ...
    return ngx_quiche_new_connection(c, conf, &pkt);
}
