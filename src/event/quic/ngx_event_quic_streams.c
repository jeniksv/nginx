...
ngx_quic_open_stream(ngx_connection_t *c, ngx_uint_t bidi)
{
    ...
    streams_left = qc->streams.server_streams_left(qc, bidi);
...
{
    return c->quic->reset(c->quic, err);
}
...
ngx_quic_shutdown_stream(ngx_connection_t *c, int how)
{
    ...
    if (how == NGX_RDWR_SHUTDOWN || how == NGX_WRITE_SHUTDOWN) {
        if (qs->shutdown_send(c) != NGX_OK) {
            return NGX_ERROR;
        }
    }

    if (how == NGX_RDWR_SHUTDOWN || how == NGX_READ_SHUTDOWN) {
        if (qs->shutdown_recv(c) != NGX_OK) {
            return NGX_ERROR;
        }
    }
...
ngx_quic_init_stream(ngx_quic_stream_t *qs)
{
    ...
    qs->close = ngx_quic_close_stream;
    qs->reset = ngx_quic_do_reset_stream;
    qs->shutdown_recv = ngx_quic_shutdown_stream_recv;
    qs->shutdown_send = ngx_quic_shutdown_stream_send;
...
ngx_uint_t
ngx_quic_server_streams_left(ngx_quic_connection_t *qc, ngx_uint_t bidi)
{
    if (bidi) {
        return qc->streams.server_streams_bidi
               < qc->streams.server_max_streams_bidi;
    }

    return qc->streams.server_streams_uni
...
void
ngx_quiche_init_stream(ngx_quic_stream_t  *qs)
{
    ngx_connection_t   *sc;
...
ngx_uint_t
ngx_quiche_server_streams_left(ngx_quic_connection_t *qc, ngx_uint_t bidi)
{
    if (bidi) {
